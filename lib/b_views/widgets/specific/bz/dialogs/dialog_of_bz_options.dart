...
class DialogOfBzOptions extends StatefulWidget {
  ...
  static Future<void> show(
      {BuildContext context, BzModel bzModel, UserModel userModel}) async {
    const double _spacing = DialogOfBzOptions.buttonHeight * 0.1;
    const double _height =
        (buttonHeight * numberOfButtons) + (_spacing * numberOfButtons) + 30;
...
// -----------------------------------------------------------------------------
  Future<void> _deleteBzOnTap(BzModel bzModel) async {
    ...
    final bool _dialogResult = await CenterDialog.showCenterDialog(
      ...
      title: '',
      body:
          'Are you sure you want to Delete ${bzModel.name} Business account ?',
      boolDialog: true,
    );
...
// -----------------------------------------------------------------------------
  Future<void> _deactivateBzOnTap(BzModel bzModel) async {
    ...
    final bool _dialogResult = await CenterDialog.showCenterDialog(
      ...
      title: '',
      body:
          'Are you sure you want to Deactivate ${bzModel.name} Business account ?',
      boolDialog: true,
    );
