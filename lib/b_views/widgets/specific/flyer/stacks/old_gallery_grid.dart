...
// -----------------------------------------------------------------------------
  static int gridColumnCount(int flyersLength) {
    final int _gridColumnsCount = flyersLength > 12
        ? 3
        : flyersLength > 6
            ? 2
            : 2;
...
// -----------------------------------------------------------------------------
  static double gridFlyerBoxWidth({double gridZoneWidth, int flyersLength}) {
    ...
    final double _gridFlyerWidth = gridZoneWidth /
        (_gridColumnsCount +
            (_gridColumnsCount * _spacingRatioToGridWidth) +
...
// -----------------------------------------------------------------------------
  static double gridSpacing({double gridZoneWidth, int flyersLength}) {
    final double _gridFlyerWidth = gridFlyerBoxWidth(
        gridZoneWidth: gridZoneWidth, flyersLength: flyersLength);
...
// -----------------------------------------------------------------------------
  static int numOfRows(int flyersLength) {
    final int _gridColumnsCount = gridColumnCount(flyersLength);
    return (flyersLength / _gridColumnsCount).ceil();
  }

// -----------------------------------------------------------------------------
  static double gridHeight(
      {BuildContext context,
      double gridZoneWidth,
      bool gridHasAddButton,
      int flyersLength}) {
    ...
    final double _gridHeight = _gridFlyerHeight *
        (_numOfRows +
            (_numOfRows * _spacingRatioToGridWidth) +
            _spacingRatioToGridWidth);
    return _gridHeight;
  }
...
// -----------------------------------------------------------------------------
    final int _flyersLength =
        addButtonIsOn == true ? _gridFlyers.length + 1 : _gridFlyers.length;

    /// TASK : THERE IS SOMETHING WRONG WITH THE BELOW -5 PIXELS !
    final double _gridFlyerWidth = gridFlyerBoxWidth(
            gridZoneWidth: gridZoneWidth, flyersLength: _flyersLength) -
        5;
    final double _gridSpacing =
        gridSpacing(gridZoneWidth: gridZoneWidth, flyersLength: _flyersLength);
// -----------------------------------------------------------------------------
    ...
    return SizedBox(
      ...
      child: GridView(
        ...
        key: PageStorageKey<String>(
            'gridFlyers_$bzID'), //new Key(loadedFlyers[flyerIndex].f01flyerID),
        gridDelegate:
        ...
        children: <Widget>[
          ...
          ...List<Widget>.generate(
              _gridFlyers.length,
              (int index) => Opacity(
                    opacity: flyersVisibilities == null ||
                            flyersVisibilities.isEmpty ||
