...
import 'package:bldrs/b_views/screens/i_flyer/flyer_maker_screen.dart/flyer_maker_screen.dart';
import 'package:bldrs/b_views/screens/i_flyer/flyer_maker_screen.dart/keywords_picker_screen.dart';
...
// -----------------------------------------------------------------------------
  ...
  void didUpdateWidget(covariant FinalFlyer oldFlyer) {
    if (oldFlyer.flyerID != widget.flyerID ||
...
// -----------------------------------------------------o
  SuperFlyer _initializeSuperFlyer() {
    ...
    if (widget.flyerModel != null) {
      _superFlyer = SuperFlyer.createEmptySuperFlyer(
          flyerBoxWidth: widget.flyerBoxWidth,
          goesToEditor: widget
              .goesToEditor); //_getSuperFlyerFromFlyer(flyerModel: widget.flyerModel);
  ...
  SuperFlyer _getSuperFlyerFromFlyer({
    ...
    if (flyerModel != null) {
      _superFlyer = SuperFlyer.createViewSuperFlyerFromFlyerModel(
        ...
        onVerticalPageBack: () async {
          await _slideBackToSlidesPage();
        },
        onHeaderTap: (bool isExpanded) {
          _onHeaderTap(isExpanded);
        },
        onSlideRightTap: _onSlideRightTap,
        onSlideLeftTap: _onSlideLeftTap,
        onSwipeFlyer: (Sliders.SwipeDirection direction) =>
            widget.onSwipeFlyer(direction),
        onTinyFlyerTap: () async {
          await _openTinyFlyer();
        },
        ...
        onShowReviewOptions: (ReviewModel review) async {
          await _onShowReviewOptions(review);
        },
    ...
    /// TASK : below code is temp,,, should see what to do if flyer not found on db
      ...
      _superFlyer = SuperFlyer.createEmptySuperFlyer(
          flyerBoxWidth: widget.flyerBoxWidth, goesToEditor: false);
    }
  ...
  Future<SuperFlyer> _getDraftSuperFlyerFromFlyer({
    ...
    if (flyerModel != null && bzModel != null) {
      _superFlyer = await SuperFlyer.createDraftSuperFlyerFromFlyer(
        ...
        onAboutTap: () async {
          await _onMoreInfoTap();
        },
        onKeywordsTap: () async {
          await _onAddKeywords();
        },
        onSpecsTap: () async {
          await _onAddSpecs();
        },
        ...
        onPublishFlyer: () async {
          await _onPublishFlyer();
        },
        onDeleteFlyer: () async {
          await _onDeleteFlyer();
        },
        onUnPublishFlyer: () async {
          await _onUnpublishFlyer();
        },
        onRepublishFlyer: () async {
          await _onRepublishFlyer();
        },
        onSaveInfoScrollOffset: onSaveInfoScrollOffset,
        getInfoScrollOffset: getInfoScrollOffset,
        initialInfoScrollOffset: getInfoScrollOffset(),
...
// -----------------------------------------------------o
  Future<void> _goToFlyerEditor({BuildContext context, bool firstTimer}) async {
    blog(
        'going to flyer editor for flyerID ${_superFlyer.flyerID} as firstTimer is $firstTimer');
...
// -----------------------------------------------------------------------------
  ...
  void _onHorizontalSlideSwipe(int newIndex) {
    ...
    final Sliders.SwipeDirection _direction = Animators.getSwipeDirection(
      newIndex: newIndex,
      oldIndex: _superFlyer.currentSlideIndex,
    );
...
// -----------------------------------------------------o
  Future<void> _slideBackToSlidesPage() async {
    blog(
        '_slideBackToSlidesPage : sliding from info page to slides page aho by tap');
...
// -----------------------------------------------------o
  Future<void> _onAnkhTap() async {
    ...
    unawaited(TopDialog.showTopDialog(
      context: context,
      verse:
          _superFlyer.rec.ankhIsOn == true ? 'Flyer is unsaved' : 'Flyer saved',
      // secondLine: 'Allows users to follow your account',
      color:
          _superFlyer.rec.ankhIsOn == true ? Colorz.grey255 : Colorz.yellow255,
    ));
...
// -----------------------------------------------------o
  Future<void> _onShareTap() async {
    ...
    if (widget.inEditor == true) {
      ...
      final dynamic _dynamicLink =
          await DynamicLinksApi().createFlyerDynamicLink(
        context: context,
        isShortURL: true,
        flyerModel: _flyer,
        slideIndex: _i,
                    ...
                    child: SuperTextField(
                      ...
                      onChanged: (String val) {
                        final bool _reviewControllerHasValue =
                            TextChecker.textControllerIsEmpty(
                                    _superFlyer.rec.reviewController) ==
                                false;

                        blog('_existingReview : $_existingReview');
                        blog(
                            '_reviewControllerHasValue : $_reviewControllerHasValue');
                        blog(
                            '_superFlyer.rec.reviewController.text : ${_superFlyer.rec.reviewController.text}');
                        // blog('val : $val');
                        blog('_canUploadReview : $_canUploadReview');

                        if (_reviewControllerHasValue == true) {
                          if (_superFlyer.rec.reviewController.text !=
                              _existingReview) {
                            setDialogState(() {
...
// -----------------------------------------------------o
  Future<void> _onSubmitReview({ReviewModel review}) async {
    ...
    if (TextChecker.textControllerIsEmpty(_superFlyer.rec.reviewController) ==
        true) {
      ...
      final ReviewModel _review = review ??
          ReviewModel(
            body: _superFlyer.rec.reviewController.text,
            userID: FireAuthOps.superUserID(),
            time: DateTime.now(),
...
// -----------------------------------------------------o
  void _onShowAuthorTap() {
    blog(
        'triggering showing author : as _superFlyer is : ${_superFlyer.flyerID} \n and superFlyer.flyerShowsAuthor is : ${_superFlyer.flyerShowsAuthor}');
    ...
    if (mounted) {
      ...
      if (FlyerMethod.maxSlidesReached(superFlyer: _superFlyer) == true) {
        final int _maxLength =
            Standards.getMaxSlidesCount(_superFlyer.bz.accountType);
      ...
      /// A - if can pick more gallery pictures
        ...
        final List<Asset> _assetsSources =
            MutableSlide.getAssetsFromMutableSlides(_superFlyer
                .mSlides); //Imagers.getOnlyAssetsFromDynamics(_superFlyer.assetsSources);
        /// B2 - assert that index in never null
        _superFlyer.currentSlideIndex =
            FlyerMethod.unNullIndexIfNull(_superFlyer.currentSlideIndex);

        /// B3 - pick images from gallery
        final List<dynamic> _phoneAssets =
            await Imagers.takeGalleryMultiPictures(
          context: context,
        ...
        /// B4 - if picked new assets
          ...
          for (int i = 0; i < _phoneAssets.length; i++) {
            ...
            final int _assetIndexInAssets =
                MutableSlide.getMutableSlideIndexThatContainsThisAsset(
            ...
            if (_assetFound) {
              ...
              final File _newFile =
                  await Imagers.getFileFromPickerAsset(_newAsset);
              ...
              final MutableSlide _mutableSlide = MutableSlide(
                ...
                picFit: Imagers.concludeBoxFitForAsset(
                    asset: _newAsset, flyerBoxWidth: widget.flyerBoxWidth),
          ...
          Tracer.traceSetState(
              screenName: 'FinalFlyer',
              varName: 'numberOfSlides',
              varNewValue: _superFlyer.mSlides.length);
...
// -----------------------------------------------------o
  Future<void> _onDeleteSlide() async {
    ...
    if (_superFlyer.numberOfSlides == 0 ||
      ...
      blog(
          'FinalFlyer : _onDeleteSlide : Can not delete slide : ${_superFlyer.currentSlideIndex}');
    }
    ...
    /// A - if slides are not empty
      ...
      /// B - if at (LAST) slide
      else if (_superFlyer.currentSlideIndex + 1 ==
          _superFlyer.numberOfSlides) {
...
// -----------------------------------------------------o
  Future<void> _deleteFirstSlide() async {
    blog(
        'DELETING STARTS AT (FIRST) index : ${_superFlyer.currentSlideIndex}, numberOfSlides : ${_superFlyer.numberOfSlides} ------------------------------------');

    /// 1 - if only one slide remaining
    if (_superFlyer.numberOfSlides == 1) {
      blog(
          '_draft.visibilities : ${_superFlyer.mSlides[_superFlyer.currentSlideIndex].toString()}, _draft.numberOfSlides : $_superFlyer.numberOfSlides');
    ...
    /// 2 - if two slides remaining
      ...
      await Future<void>.delayed(Ratioz.durationFading210, () async {
        /// C - slide
        await Sliders.slideToNext(_superFlyer.nav.horizontalController,
            _superFlyer.numberOfSlides, _superFlyer.currentSlideIndex);
    ...
    blog(
        'DELETING ENDS AT (FIRST) : index : ${_superFlyer.currentSlideIndex}, numberOfSlides : ${_superFlyer.numberOfSlides} ------------------------------------');
  }

// -----------------------------------------------------o
  Future<void> _deleteMiddleOrLastSlide() async {
    blog(
        'XXXXX ----- DELETING STARTS AT (MIDDLE) index : ${_superFlyer.currentSlideIndex}, numberOfSlides : ${_superFlyer.numberOfSlides}');
    ...
    await Future<void>.delayed(Ratioz.durationFading210, () async {
      ...
      await Sliders.slideToBackFrom(
          _superFlyer.nav.horizontalController, _originalIndex);
    ...
    blog(
        'XXXXX -------  DELETING ENDS AT (MIDDLE) : index : ${_superFlyer.currentSlideIndex}, numberOfSlides : ${_superFlyer.numberOfSlides}');
  }
  ...
  void _statelessTriggerSlideVisibility(int index) {
    if (index != null) {
      if (index >= 0 && _superFlyer.mSlides.isNotEmpty) {
        blog(
            '_superFlyer.mSlides[index].isVisible was ${_superFlyer.mSlides[index].opacity} for index : $index');
        ...
        blog(
            '_superFlyer.mSlides[index].isVisible is ${_superFlyer.mSlides[index].opacity} for index : $index');
      } else {
...
// -----------------------------------------------------o
  void _statelessSlideDelete(int index) {
    blog(
        'before stateless delete index was $index, _draft.numberOfSlides was : ${_superFlyer.numberOfSlides}');
    ...
    blog(
        'after stateless delete index is $index, _draft.numberOfSlides is : ${_superFlyer.numberOfSlides}');
  }
...
// -----------------------------------------------------o
  Future<void> _onCropImage() async {
    if (_superFlyer.mSlides.isNotEmpty) {
      ...
      if (croppedFile != null) {
        setState(() {
          _superFlyer.mSlides[_superFlyer.currentSlideIndex].picFile =
              croppedFile;
        });
      }
  ...
  Future<void> _onResetImage() async {
    if (_superFlyer.mSlides.isNotEmpty) {
      if (_superFlyer.mSlides[_superFlyer.currentSlideIndex].picAsset != null) {
        final File _file = await Imagers.getFileFromPickerAsset(
            _superFlyer.mSlides[_superFlyer.currentSlideIndex].picAsset);
...
// -----------------------------------------------------o
  void _onFitImage() {
    ...
    blog(
        'tapping on fit image : ${_superFlyer.mSlides.length} mSlides and _currentPicFit was : $_currentPicFit');
    ...
    blog(
        'tapping on fit image : ${_superFlyer.mSlides.length} mSlides and _currentPicFit is : $_currentPicFit');
  }
  ...
  Future<void> _onChangeZone() async {
      ...
      child: BottomDialogButtons(
        ...
        buttonTap: (String countryID) async {
          ...
          setState(() {
            ...
            _citiesMaps = CityModel.getCitiesNamesMapModels(
                context: context, cities: _selectedCountryCities);
        ...
        child: BottomDialogButtons(
          ...
          buttonTap: (String cityID) async {
            ...
            setState(() {
              ...
              _districtsMaps = DistrictModel.getDistrictsNamesMapModels(
                  context: context, districts: _districts);
    ...
    if (_openNextDialog == true) {
      await BottomDialog.showBottomDialog(
        context: context,
        draggable: true,
        title: '${CityModel.getTranslatedCityNameFromCity(
          context: context,
          city: _city,
        )} Districts',
        child: BottomDialogButtons(
...
// -----------------------------------------------------o
  Future<void> _onPublishFlyer() async {
    ...
    await CenterDialog.showCenterDialog(
      context: context,
      title: 'Great !',
      body: _superFlyer.edit.firstTimer == true
          ? 'Flyer has been created'
  ...
  Future<FlyerModel> _createNewFlyer() async {
    ...
    if (_inputsAreValid == false) {
      ...
      final List<SlideModel> _slides =
          await _createSlidesFromCurrentSuperFlyer();
      ...
      final FlyerModel _newFlyerModel = FlyerModel(
        ...
        /// TASK : redundancy : will be created in createFlyerOps
        title: _superFlyer.title,
        trigram: TextGen.createTrigram(input: _superFlyer.title),
        ...
        times: <PublishTime>[
          PublishTime(time: DateTime.now(), state: FlyerState.published)
        ],
  ...
  Future<FlyerModel> _updateExistingFlyer(FlyerModel originalFlyer) async {
    ...
    if (_inputsAreValid == false) {
      ...
      final FlyerModel _tempUpdatedFlyerModel = FlyerModel(
        ...
        /// IS NOT RECREATED IN UPDATE FLYER OPS
        // -------------------------
        flyerType: _superFlyer.flyerType,
        flyerState: _superFlyer.flyerState,
        keywordsIDs: KW.getKeywordsIDsFromKeywords(_superFlyer.keywords),
