...
int lastTwoIntegersFromAString(String string) {
  final String _lastTwoSubStrings =
      TextMod.lastTwoSubStringsFromAString(string);
...
int createUniqueIndex(
    {@required List<int> existingIndexes, int maxIndex = 999999}) {
  final Random _random = Random();
...
List<dynamic> createListWithDummyValue(
    {@required int length, @required int value}) {
...
double getFractions({@required double number, int fractionDigits}) {
  final String _numberAsString = fractionDigits == null
      ...
      : getFractionStringWithoutZero(
          fraction: number, fractionDigits: fractionDigits);
  final String _fractionsString =
      TextMod.removeTextBeforeLastSpecialCharacter(_numberAsString, '.');
...
String getFractionStringWithoutZero(
    {@required double fraction, int fractionDigits}) {
  final String _fractionAsString = fraction.toString();
  String _fractionAsStringWithoutZero =
      TextMod.removeTextBeforeLastSpecialCharacter(_fractionAsString, '.');
...
int getNumberOfFractions({@required double number}) {
  final double _numberFraction =
      getFractions(number: number, fractionDigits: 100);
  final String _numberFractionsString =
      TextMod.removeTextBeforeFirstSpecialCharacter(
          _numberFraction.toString(), '.');
  blog(
      'getNumberOfFractions : _numberFractionsString : $_numberFractionsString');
...
int discountPercentage(
    {@required double oldPrice, @required double currentPrice}) {
...
List<int> getRandomIndexes(
    {@required int numberOfIndexes, @required int maxIndex}) {
  final List<int> _indexes = <int>[];
  for (int i = 0; i < numberOfIndexes; i++) {
    final int _newIndex =
        createUniqueIndex(existingIndexes: _indexes, maxIndex: maxIndex);
