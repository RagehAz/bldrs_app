...
// -----------------------------------------------------------------------------
String sectionDescriptionStringer(
    BuildContext context, SectionClass.Section section) {
  final String _description = section == SectionClass.Section.properties
      ? 'By RealEstate Developers & Brokers.'
      ...
      section == SectionClass.Section.designs
          ...
          section == SectionClass.Section.projects
              ...
              : //Wordz.constructionTagLine(context) :
              section == SectionClass.Section.crafts
                  ? 'By Craftsmen, Technicians & Artists.'
                  : //Wordz.constructionTagLine(context) :
...
String bzTypePluralStringer(BuildContext context, BzType bzType) {
  return bzType == BzType.developer
      ? Wordz.realEstateDevelopers(context)
      : bzType == BzType.broker
          ? Wordz.brokers(context)
          : bzType == BzType.designer
...
// -----------------------------------------------------------------------------
String flyerTypeSingleStringer(
    BuildContext context, FlyerTypeClass.FlyerType flyerType) {
  return flyerType == FlyerTypeClass.FlyerType.property
...
String flyerTypeSingleStringerByBzType(BuildContext context, BzType bzType) {
  final FlyerTypeClass.FlyerType _defaultFlyerType =
      FlyerTypeClass.concludeFlyerType(bzType);
  final String _string = flyerTypeSingleStringer(context, _defaultFlyerType);
  return _string;
}

// -----------------------------------------------------------------------------
String flyerTypePluralStringer(
    BuildContext context, FlyerTypeClass.FlyerType flyerType) {
  return flyerType == FlyerTypeClass.FlyerType.property
...
String countryStringer(
  ...
  if (country != null && ZoneModel.zoneHasAllIDs(zone)) {
    ...
    _verse = zone.countryID == null || zone.cityID == null
        ? '...'
...
String cityCountryStringer(
  ...
  if (country != null && ZoneModel.zoneHasAllIDs(zone)) {
    final String _countryName =
        Name.getNameByCurrentLingoFromNames(context, country.names);
...
String functionStringer(Function function) {
  ...
  return _functionNameAsAString.substring(
      _s + 1, _e); // return functionNameAsAString;
}

// -----------------------------------------------------------------------------
String askHinter(BuildContext context, BzType bzType) {
  final String _askHint = bzType == BzType.developer
...
List<String> createTrigram({@required String input}) {
  ...
  if (input != null) {
    ...
    for (int trigramLength = 3;
        trigramLength <= _maxTrigramLength;
        trigramLength++) {
      ...
      for (int i = 0; i < _charactersLength - _difference; i++) {
        ...
        /// 7 - add combination
        _trigram = TextMod.addStringToListIfDoesNotContainIt(
          strings: _trigram,
