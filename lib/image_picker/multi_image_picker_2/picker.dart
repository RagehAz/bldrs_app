...
class MultiImagePicker {
  ...
  MethodChannel('multi_image_picker');
  ...
  static Future<List<Asset>> pickImages({
    ...
    if (maxImages < 0) {
      throw ArgumentError.value(maxImages, 'maxImages cannot be negative');
    }
  ...
  ///
  /// The actual image data is sent via BinaryChannel.
  static Future<bool> requestOriginal(String identifier, dynamic quality) async {
    try {
      final bool ret =
  ...
  static Map<String, dynamic> _normalizeMetadata(Map<String, dynamic> json) {
    ...
    json.forEach((String metaKey, dynamic metaValue) {
      if (metaKey == '{Exif}' || metaKey == '{TIFF}') {
        ...
        final Map gpsMap = <String, dynamic>{};
        final Map<String, dynamic> metaMap = Map<String, dynamic>.from(metaValue);
