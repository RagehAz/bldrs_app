...
import 'package:bldrs/ambassadors/database/db_bond.dart';
...
import 'package:bldrs/ambassadors/database/db_locations/db_bz_location.dart';
import 'package:bldrs/ambassadors/database/db_locations/db_flyer_location.dart';
import 'package:bldrs/ambassadors/database/db_locations/db_user_location.dart';
import 'package:bldrs/ambassadors/database/db_origin.dart';
...
import 'package:bldrs/ambassadors/database/db_slide_ref.dart';
...
import 'package:bldrs/models/old_models_to_delete_when_done/reference_model.dart';
...
List<BondModel>       xBonds            = emptyDB? [] : dbBonds;
List<BzModel>         xBzz              = emptyDB? [] : dbBzz;
List<LocationModel>   xBzzLocations     = emptyDB? [] : dbBzzLocations;
...
List<LocationModel>   xFlyersLocations  = emptyDB? [] : dbFlyersLocations;
List<FollowModel>     xFollows          = emptyDB? [] : dbFollows;
List<HorusModel>      xHorusee          = emptyDB? [] : dbHorusee;
List<OriginModel>     xOrigins          = emptyDB? [] : dbOrigins;
List<ReferenceModel>  xRefs             = emptyDB? [] : dbRefs;
List<SaveModel>       xSaves            = emptyDB? [] : dbSaves;
List<ShareModel>      xShares           = emptyDB? [] : dbShares;
List<SlideModel>      xSlides           = emptyDB? [] : dbSlides;
List<UserModel>       xUsers            = emptyDB? [] : dbUsers;
List<LocationModel>   xUsersLocations   = emptyDB? [] : dbUsersLocations;
...
CoUser getCoUserByUserID(String userID){
  ...
  LocationModel userLocation = getUserLocationByUserID(userID);
...
List<CoAuthor> getAllCoAuthors(){
  ...
  xAuthors.forEach((au) {
    allCoAuthors.add(getCoAuthorByAuthorID(au.authorID));
  });
  return allCoAuthors;
}
// ----------------------------------------------------------------------------
...
CoSlide getCoSlideBySlideID(String slideID){
  ...
  List<OriginModel> origins = getOriginsBySlideID(slideID);
  List<BondModel> bonds = getBondsBySlideId(slideID);
