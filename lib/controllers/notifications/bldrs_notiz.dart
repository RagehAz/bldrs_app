  ...
  static NotiModel newPublishedFlyer({@required BzModel bzModel, @required FlyerModel flyer}){
    ...
    return
      NotiModel(
        ...
        title: 'New flyer',
        timeStamp: DateTime.now(),
        body: '${bzModel.name} published a new ${flyer.flyerType.toString()}flyer', // TASK : fix this flyer type localized in notification
        attachment: flyer.id,
  ...
  static NotiModel authorInvitationCC({@required AuthorModel sender, @required UserModel reciever, @required BzModel tinyBz}){
    return
      NotiModel(
        ...
        timeStamp: DateTime.now(),
        body: 'An invitation has been sent for ${reciever.name} to join the team of ${tinyBz.name}',
