...
class Chain {
  ...
  static dynamic _cipherSonsOLD(dynamic sons) {
    ...
    final bool _sonsArePhids = Keyworder.checkIsPhids(sons);
  ...
  static bool checkChainsSonsAreIdentical({
    ...
    final bool sonsAisPhids = Keyworder.checkIsPhids(chain1?.sons);
    ...
    final bool sonsBIsPhids = Keyworder.checkIsPhids(chain2?.sons);
  ...
  static bool checkChainIncludeThisPhid({
    ...
    if (chain != null && phid != null) {
      ...
      else if (Keyworder.checkIsPhids(chain.sons) == true) {
  ...
  void blogChain({
    ...
    if (id != null) {
      ...
      } else if (Keyworder.checkIsPhids(sons) == true) {
  ...
  static void blogSon(dynamic son) {
    ...
    final bool _isPhid = Keyworder.checkIsPhid(son);
    final bool _isPhids = Keyworder.checkIsPhids(son);
  ...
  static List<String> getOnlyPhidsSonsFromChain({
    ...
    if (chain != null) {
      ...
      if (Keyworder.checkIsPhids(chain.sons) == true) {
  ...
  static Chain? sortChainSonsByIndex(Chain? chain){
    ...
    if (chain != null){
      ...
      final bool _isPhids = Keyworder.checkIsPhids(chain.sons);
  ...
  static List<Chain> sortChainsByIndexes(List<Chain>? input){
    ...
    if (Lister.checkCanLoop(input) == true){
      ...
      _sortedChainsIDs.sort((a, b){
        final int _indexA = Phider.getIndexFromPhid(a.id) ?? 0;
        final int _indexB = Phider.getIndexFromPhid(b.id) ?? 0;
  ...
  static List<String> sortPhidsByIndexes(List<String> input){
    ...
    if (Lister.checkCanLoop(input) == true){
      ...
      _phids.sort((a, b){
        final int _indexA = Phider.getIndexFromPhid(a) ?? 0;
        final int _indexB = Phider.getIndexFromPhid(b) ?? 0;
        return _indexA.compareTo(_indexB);
      });
  ...
  static Chain? createChainIndexes({
    ...
    if (chain != null){
      ...
      final String? _chainID = Phider.addIndexToPhid(
        phid: chain.id,
        index: chainIndex,
        // overrideExisting: true,
      );
  ...
  static List<String> createPhidsIndexes(List<String> phids){
    ...
    if (Lister.checkCanLoop(phids) == true){
      ...
      for (int i = 0; i< phids.length; i++){
        ...
        final String? _modified = Phider.addIndexToPhid(
            phid: Phider.removeIndexFromPhid(phid: phids[i]),
            index: i
        );
  ...
  static String? getPossibleID(dynamic son){
    ...
    if (son != null){
      ...
      if (_isChains){
        ...
        final Chain chain = son;
        _id = Phider.removeIndexFromPhid(phid: chain.id);
      }
      else if (_isPhids){

      }
      else if (_isPhid){
        _id = Phider.removeIndexFromPhid(phid: son);
      }
      else if (_isDataCreator){
