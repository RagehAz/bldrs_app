...
// -----------------------------------------------------------------------------
  static Map<String, dynamic> cipherNames(
      {@required List<Name> names, bool addTrigrams = true}) {
...
// -----------------------------------------------------------------------------
  static String getNameByCurrentLingoFromNames(
...
// -----------------------------------------------------------------------------
  static String getNameByLingoFromNames(
      {@required List<Name> names, @required String lingoCode}) {
    String _nameValue;

    if (Mapper.canLoopList(names)) {
      final Name _foundName = names.singleWhere(
          (Name name) => name.code == lingoCode,
          orElse: () => null);

      if (_foundName == null) {
        _nameValue = names
            .singleWhere((Name name) => name.code == Lingo.englishCode)
            ?.value;
      } else {
...
// -----------------------------------------------------------------------------
  static bool namesIncludesValueForThisLanguage(
      {@required List<Name> names, @required String lingoCode}) {
...
// -----------------------------------------------------------------------------
  /// TASK : TEST THIS
  static bool namesAreTheSame(
      {@required Name firstName, @required Name secondName}) {
    bool _namesAreTheSame = false;

    if (firstName != null && secondName != null) {
      if (firstName.code == secondName.code) {
        if (firstName.value == secondName.value) {
          if (Mapper.listsAreTheSame(
              list1: firstName.trigram, list2: secondName.trigram)) {
...
// -----------------------------------------------------------------------------
  /// TASK : TEST THIS
  static bool namesListsAreTheSame(
      {@required List<Name> firstNames, @required List<Name> secondNames}) {
      ...
      if (firstNames.length == secondNames.length) {
        ...
        for (final String code in codes) {
          final String firstName =
              getNameByLingoFromNames(names: firstNames, lingoCode: code);
          final String secondName =
              getNameByLingoFromNames(names: secondNames, lingoCode: code);
...
// -----------------------------------------------------------------------------
  static void printNames(List<Name> names) {
    ...
    if (Mapper.canLoopList(names)) {
      for (final Name name in names) {
        blog(
            'code : [ ${name.code} ] : name : [ ${name.value} ] : trigramLength : ${name.trigram?.length}');
      }
    }
