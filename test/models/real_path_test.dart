...
void main() {
  ...
  group('cipherDayNodeName', () {
    ...
    test('Returns null when dateTime is null', () {
      final result = RealPath.cipherDayNodeName(dateTime: null);
      expect(result, isNull);
    });

    test('Formats the date correctly when all components are valid', () {
      final result = RealPath.cipherDayNodeName(dateTime: DateTime(2023, 9, 1));
      expect(result, 'd_2023_09_01');
    });

    test('Handles single-digit month and day values', () {
      final result = RealPath.cipherDayNodeName(dateTime: DateTime(2023, 3, 5));
      expect(result, 'd_2023_03_05');
    });

    test('Handles single-digit year value', () {
      final result = RealPath.cipherDayNodeName(dateTime: DateTime(23, 12, 31));
      expect(result, 'd_0023_12_31');
    });

    test('Handles leap year correctly', () {
      final result = RealPath.cipherDayNodeName(dateTime: DateTime(2024, 2, 29));
      expect(result, 'd_2024_02_29');
    });

    test('Handles edge case with minimum year value', () {
      final result = RealPath.cipherDayNodeName(dateTime: DateTime(1, 1, 1));
      expect(result, 'd_0001_01_01');
    });

    test('Handles zero year correctly', () {
      final result = RealPath.cipherDayNodeName(dateTime: DateTime(0, 5, 1));
  ...
  group('decipherDayNodeName', () {
    test('Returns null when nodeName is null', () {
      final result = RealPath.decipherDayNodeName(nodeName: null);
      expect(result, isNull);
    });

    test('Handles invalid nodeName format', () {
      final result = RealPath.decipherDayNodeName(nodeName: 'invalid_format');
      expect(result, isNull);
    });

    test('Deciphers nodeName with valid format', () {
      final result = RealPath.decipherDayNodeName(nodeName: 'd_2023_09_01');
      expect(result, DateTime(2023, 9, 1));
    });

    test('Handles single-digit month and day values', () {
      final result = RealPath.decipherDayNodeName(nodeName: 'd_2023_03_05');
      expect(result, DateTime(2023, 3, 5));
    });

    test('Handles single-digit year value', () {
      final result = RealPath.decipherDayNodeName(nodeName: 'd_0023_12_31');
      expect(result, DateTime(23, 12, 31));
    });

    test('Handles leap year correctly', () {
      final result = RealPath.decipherDayNodeName(nodeName: 'd_2024_02_29');
      expect(result, DateTime(2024, 2, 29));
    });

    test('Handles edge case with minimum year value', () {
      final result = RealPath.decipherDayNodeName(nodeName: 'd_0001_01_01');
  ...
  group('recorders_users_userID_date_records', () {
    test('Generates the correct path for a valid user ID', () {
      ...
      final result = RealPath.recorders_users_userID_records_date(userID: userID);
      const expectedPath =
